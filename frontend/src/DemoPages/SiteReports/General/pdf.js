import React from "react";
import jsPDF from "jspdf";
import "jspdf-autotable";
import axios from "axios";
import { ToastContainer, toast } from "react-toastify";
import Config from "../../../config/Config";
const styles = {
  fontFamily: "sans-serif",
  textAlign: "center",
};
let logo =
  "data:image/png;base64,";
class App extends React.Component {
  componentWillMount() {
    console.log("pdf propd", this.props);
    this.print();
  }
  print = async () => {
    const {
      metric,
      footfallreportGenerate,
      dwellreportGenerate,
      heatmapreportGenerate,
      pathreportGenerate,
    } = this.props;
    const pdf = new jsPDF("p", "mm", "a4");
    metric.includes("count") &&
      footfallreportGenerate &&
      pdf.text(65, 40, "FOOTFALL ANALYSIS") &&
      pdf.addImage(this.props.foofallUrl, "PNG", 10, 55, 185.89, 90.28);
    let columns = [],
      data = [],
      ans = [];

    columns = ["key", "Time", "Zone", "Total Visitors"];
    (await footfallreportGenerate) && this.props.footfalldata.length
      ? this.props.footfalldata.map((i) => {
          data = [i.key, i.hour, i.zone, i.count];
          ans.push(data);
        })
      : (ans = []);
    (await footfallreportGenerate) &&
      this.props.footfalldata.length &&
      pdf.autoTable({
        margin: { top: 160 },
        columns: columns,
        body: ans,
      });

    // dwell time
    metric.includes("count") && footfallreportGenerate && pdf.addPage();
    metric.includes("passerBy") &&
      dwellreportGenerate &&
      pdf.text(65, 40, "DWELL TIME ANALYSIS") &&
      pdf.addImage(this.props.dwellUrl, "PNG", 10, 50, 185.89, 90.28);

    let dwellcolumns = [],
      dwellDataval = [],
      dwellans = [];
    dwellcolumns = ["key", "Time", "Zone", "Passers By", "Avg. TimeSpent"];
    (await metric.includes("passerBy")) &&
    dwellreportGenerate &&
    this.props.dwelldata.length
      ? this.props.dwelldata.map((i) => {
          dwellDataval = [i.key, i.hour, i.zone, i.count, i.noOfPasser];
          dwellans.push(dwellDataval);
        })
      : (ans = []);
    // metric.includes("passerBy") && dwellreportGenerate && pdf.addPage();

    dwellreportGenerate &&
      pdf.autoTable({
        margin: { top: 160 },
        columns: dwellcolumns,
        body: dwellans,
      });

    // path tracking
    (await metric.includes("passerBy")) &&
      (await heatmapreportGenerate) &&
      pdf.addPage();
    metric.includes("pathtracking") &&
      pathreportGenerate &&
      (await pdf.text(65, 40, "PATH TRACKING ANALYSIS"));
    for (let i = 0; i < this.props.pathTrackingUrl.length; i++) {
      // pathreportGenerate &&
      await pdf.addImage(
        this.props.pathTrackingUrl[i].img,
        "PNG",
        10,
        55,
        185.89,
        125.28
      );
      pdf.setFontSize(15);
      // pathreportGenerate &&
      await pdf.text(15, 50, `Date : ${this.props.pathTrackingUrl[i].time}`);
      pdf.addPage();
    }

    //Heatmap
    metric.includes("pathtracking") && pathreportGenerate && pdf.addPage();

    (await metric.includes("heatmap")) &&
      (await heatmapreportGenerate) &&
      pdf.text(65, 40, "HEATMAP ANALYSIS");
    for (let i = 0; i < this.props.heatmapUrl.length; i++) {
      (await heatmapreportGenerate) &&
        (await pdf.addImage(
          this.props.heatmapUrl[i].img,
          "PNG",
          10,
          55,
          185.89,
          125.28
        ));
      pdf.setFontSize(15);
      // pdf.addImage(logo, "PNG", 10, 10, 95, 30);
      (await heatmapreportGenerate) &&
        (await pdf.text(15, 50, `Date : ${this.props.heatmapUrl[i].time}`));
      pdf.addPage();
    }

    pdf.setFont("helvetica");
    pdf.setFontType("bold");
    pdf.setFontSize(14);
    const pageCount = pdf.internal.getNumberOfPages();

    // For each page, print the page number and the total pages
    for (var i = 1; i <= pageCount; i++) {
      // Go to page i
      pdf.setPage(i);
      pdf.line(10, 25, 200, 25);

      // pdf.line(10, 20, 200, 20);
      pdf.line(10, 280, 200, 280);
      pdf.addImage(logo, "PNG", 10, 10, 28, 13);
      pdf.setLineWidth(1.5);
    }
    pdf.setFontSize(14);
    var res = pdf.output("datauristring");

    // save in browser
    pdf.save("Report.pdf");

    // mail send
    res.length &&
      (await axios
        .post(`${Config.hostName}/api/pdf/mailsender`, { data: res })
        .then((res) => {
          if (res.status === 200) {
            toast.success("Mail Sent", {
              position: "top-right",
              autoClose: 5000,
              hideProgressBar: true,
              closeOnClick: true,
              pauseOnHover: true,
              draggable: true,
            });
            setTimeout(() => {
              window.location.reload();
            }, 500);
          } else
            toast.error("Network Error", {
              position: "top-right",
              autoClose: 5000,
              hideProgressBar: true,
              closeOnClick: true,
              pauseOnHover: true,
              draggable: true,
            });
        }));
  };
  render() {
    return <div style={styles}></div>;
  }
}
export default App;
