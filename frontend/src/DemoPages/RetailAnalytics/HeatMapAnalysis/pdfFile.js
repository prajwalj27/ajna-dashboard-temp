...
// import FootfallAnalysis from "./StaticFootfall";
...
class App extends React.Component {
  ...
  print = () => {
    // const string = renderToString(<FootfallAnalysis url={this.props.url} />);
    const pdf = new jsPDF("l", "pt", "a4");
    // const pdf = new jsPDF("l", "pt", "ledger");
    let width = pdf.internal.pageSize.width;
    let height = pdf.internal.pageSize.height;
    pdf.text(85, 45, "Heatmap Analysis");
    pdf.addImage(this.props.url, "PNG", 0, 0, width, height);
    pdf.setFont("helvetica");
    pdf.setFontType("bold");
    pdf.setFontSize(9);
    // pdf.fromHTML(string, 25, 850, { width: 960 });
    ...
    for (var i = 1; i <= pageCount; i++) {
      // Go to page i
      pdf.setPage(i);
      pdf.addImage(logo, "PNG", 10, 15, 95, 30);
