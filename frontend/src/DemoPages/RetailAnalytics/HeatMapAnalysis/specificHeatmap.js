...
class Heatmap extends React.Component {
  ...
  fetchData = async (id, type, camera) => {
    await this.setState({ people: [], loader: true });
    await this.HeatMap();
    let changed = JSON.stringify(this.state.date);
    ...
    axios
      .post(
        `${Config.hostName}/api/heatmap/dates/${id}/${type}`,
        {
      ...
      .then(async (res) => {
        let arr = [];
        if (res.data && res.data.Coordinates && res.data.Coordinates.length) {
          res.data.Coordinates.map((i) => {
            let obj = {};
            obj.x = i.x;
            obj.y = i.y;
            arr.push(obj);
  ...
  getCameraConfig = async (id, type, camera) => {
    try {
      await this.setState({ fetchedImage: "" });
      await axios
        .get(
        ...
        .then(async (res) => {
          if (res.data) {
            await this.setState({ fetchedImage: res.data });
          }
        });
    } catch (err) {
      console.log("err", err);
  ...
  PusherComponent = (id) => {
    ...
    socket.on("insertHeatmap", (data) => {});
    socket.on("deleteHeatmap", (data) => {
      // console.log("deleteHeatmap data", data);
    });
    socket.on("updateHeatmap", (data) => {});
  };
  branchHandleChange = async (value) => {
    await this.setState({ branch: value });
